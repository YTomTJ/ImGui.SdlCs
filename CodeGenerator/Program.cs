...
namespace CodeGenerator {
    internal static class Program {
        ...
        static void Main(string[] args) {
            ...
            FunctionDefinition[] functions = functionsJson.Children().Select(jt => {
                ...
                // YTom test
                //if (name == "ImDrawList_GetClipRectMax") {
                //    //
                //    Console.WriteLine("");
                //}
                ...
                // YTom
                bool hasNonUdtVariants = jp.Values().Any(val => val["ov_cimguiname"]?.ToString().EndsWith("nonUDT2") ?? false);
                ...
                //    overloads = overloads.Where(val => val.ExportedName.EndsWith("_nonUDT2")).ToArray();
                //}
                ...
                using(CSharpCodeWriter writer = new CSharpCodeWriter(Path.Combine(outputPath, td.Name + ".gen.cs"))) {
                    ...
                    foreach (TypeReference field in td.Fields) {
                        ...
                        // YTom
                            ...
                            if (GetWrappedType(vectorElementType + "*", out string wrappedElementType)) {
                                writer.WriteLine($"public ImPool<{wrappedElementType}> {field.Name} => new ImPool<{wrappedElementType}>();");
                            } else {
                                if (GetWrappedType(vectorElementType, out wrappedElementType)) {
                                    vectorElementType = wrappedElementType;
                                }
                        ...
                        // YTom
                            ...
                            if (GetWrappedType(vectorElementType + "*", out string wrappedElementType)) {
                                writer.WriteLine($"public ImChunkStream<{wrappedElementType}> {field.Name} => new ImChunkStream<{wrappedElementType}>();");
                            } else {
                                if (GetWrappedType(vectorElementType, out wrappedElementType)) {
                                    vectorElementType = wrappedElementType;
                                }
                                writer.WriteLine($"public ImChunkStream<{vectorElementType}> {field.Name} => new ImChunkStream<{vectorElementType}>();");
                            }
                        } else {
            ...
            using(CSharpCodeWriter writer = new CSharpCodeWriter(Path.Combine(outputPath, "ImGui.gen.cs"))) {
                ...
                foreach (FunctionDefinition fd in functions) {
                    if (s_skippedFunctions.Contains(fd.Name)) {
                        continue;
                    }
        ...
        private static bool GetWrappedType(string nativeType, out string wrappedType) {
            if (nativeType.StartsWith("Im") && nativeType.EndsWith("*") && !nativeType.StartsWith("ImVector")) {
                ...
                // YTom
                if (nonPtrType == "ImGuiDir" || nonPtrType == "ImGuiItemFlags") {
                    wrappedType = null;
                    return false;
                }
        ...
        private static bool CorrectDefaultValue(string defaultVal, TypeReference tr, out string correctedDefault) {
            ...
            if (s_wellKnownDefaultValues.TryGetValue(defaultVal, out correctedDefault)) {
                return true;
            }
    ...
    class MethodVariant {
        ...
        public ParameterVariant[] Parameters {
            get;
        }
    ...
    class TypeReference {
        ...
        public TypeReference WithVariant(int variantIndex, EnumDefinition[] enums) {
            if (variantIndex == 0)
                ...
                return new TypeReference(Name, TypeVariants[variantIndex - 1], TemplateType, enums);
        }
    }
    ...
    class FunctionDefinition {
        ...
        public OverloadDefinition[] Overloads {
            get;
        }
        ...
        private OverloadDefinition[] ExpandOverloadVariants(OverloadDefinition[] overloads, EnumDefinition[] enums) {
            ...
            foreach (OverloadDefinition overload in overloads) {
                ...
                if (hasVariants) {
                    ...
                    for (int i = 1; i < variantCounts.Length; i++)
                        totalVariants *= variantCounts[i];
